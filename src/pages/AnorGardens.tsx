...
const specRows: { label: string; value: string; icon: JSX.Element }[] = [
  { label: 'Название', value: 'ANOR GARDENS', icon: <HomeIcon /> },
...
export default function AnorGardens() {
  ...
  const project = projects.find((p) => p.slug === 'anor-gardens')!
  const details = project.details!
  const others = projects.filter((p) => p.slug !== 'anor-gardens').slice(0, 3)

  const idx = projects.findIndex((p) => p.slug === 'anor-gardens')
  ...
  return (
    <main>
      ...
      <section className="relative h-[720px] 2xl:h-[888px] overflow-hidden bg-primary">
        <img loading="eager" decoding="async" src="/assets/anor-1.webp" alt="ANOR GARDENS" className="absolute inset-0 size-full object-cover" />
      ...
      <section className="bg-white py-[24px]">
        <Container>
          <div data-reveal="scale" className="relative h-[460px] 2xl:h-[700px] w-full overflow-hidden rounded-[5px] bg-black">
            <SkeletonImage key={gallery[active]} src={gallery[active]} alt="ANOR GARDENS — общий вид" className="absolute inset-0" imgClassName="size-full object-cover" />
        ...
        <Container>
          <div className="relative">
            <div className="h-[460px] 2xl:h-[560px] w-full overflow-hidden rounded-[5px]">
              <iframe title="ANOR GARDENS на карте" src="https://yandex.ru/map-widget/v1/?ll=69.33%2C41.33&z=14&pt=69.33%2C41.33%2Cpm2rdm" className="size-full border-0" loading="lazy" />
            </div>
        ...
        <Container>
          <div data-reveal className="flex flex-col gap-10 2xl:flex-row 2xl:items-start 2xl:justify-between">
            <div className="flex flex-col gap-5">
              <SectionTag>Почему Anor Gardens</SectionTag>
              <h2 className="font-heading text-[44px] 2xl:text-[61px] font-bold uppercase leading-[1.3] text-ink">
                Преимущества
              </h2>
            </div>
            <p className="font-vela text-[18px] 2xl:text-[20px] leading-[1.3] text-ink 2xl:w-[735px] 2xl:shrink-0">
              «Anor Gardens» — закрытый комплекс комфорт-класса на 3 гектарах: 12 малоэтажных блоков, 547 квартир,
